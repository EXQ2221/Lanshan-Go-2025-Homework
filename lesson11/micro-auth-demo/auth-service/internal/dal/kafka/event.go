...
// SendSecurityEvent 序列化并发送安全事件
func (p *Producer) SendSecurityEvent(event SecurityEvent) error {
	if event.OccurAt == 0 {
		event.OccurAt = time.Now().Unix()
	}
	data, err := json.Marshal(event)
	if err != nil {
		return err
	}
	return p.SendMessage(TopicSecurityEvent, data)
}
