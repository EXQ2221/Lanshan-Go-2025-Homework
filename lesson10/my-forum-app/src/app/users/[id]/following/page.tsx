...
export default function FollowingPage() {
  ...
  // GET /users/following/:id?page=1&size=20 响应 data.users
  const fetchFollowing = useCallback(async (pageNum: number) => {
    if (!userId || !hasMore || loading) return

    setLoading(true)
    try {
      const res = await api.get<FollowListResponse>(`/users/following/${userId}`, {
  ...
  }, [hasMore, loading, fetchFollowing, page])

  const unfollow = async (targetId: number, index: number) => {
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">我关注的</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            {users.length === 0 && !loading && <p className="text-center text-gray-500">暂无关注</p>}
            ...
            {users.map((user, index) => (
              <div key={user.id} className="flex items-center gap-4 py-3 border-b last:border-b-0">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => unfollow(user.id, index)}
                >
                  取消关注
                </Button>
              </div>
            ...
            {!hasMore && users.length > 0 && (
              <p className="text-center text-gray-500 py-4">已加载全部关注</p>
            )}
          </CardContent>
        </Card>
      </div>
